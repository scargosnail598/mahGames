...
function handle(peer,message) {
  ...
  if(message.type==="create"){
    ...
    rooms.set(roomCode,{code:roomCode,host:peer,guest:null,createdAt:Date.now()});
    ...
    room.host.send({type:"ready",role:"host",room:roomCode});peer.send({type:"ready",role:"guest",room:roomCode});
    ...
    if(message.type==="input" && peer===room.guest && Number.isFinite(message.x) && Number.isFinite(message.y))
      room.host.send({type:"input",x:Math.max(0,Math.min(1,message.x)),y:Math.max(0,Math.min(1,message.y))});
