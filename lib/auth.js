...
class AuthService {
  ...
  publicUser(row) {
    return {id:Number(row.id),displayName:row.display_name,email:row.email,avatarUrl:row.avatar_url||""};
  }
  ...
  async google(request,response) {
    ...
    const userRow=this.db.prepare("SELECT id,display_name,email,avatar_url FROM users WHERE google_sub=?").get(googleSub);
  ...
  async handle(request,response,pathname) {
    ...
    if(pathname.startsWith("/api/")){
      if(["/api/me","/api/auth/google","/api/logout"].includes(pathname))response.writeHead(405,{allow:pathname==="/api/me"?"GET":"POST","cache-control":"no-store"});
