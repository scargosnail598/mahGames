(function () {
  ...
  class CoopClient{
    constructor(game){
      ...
      this.controlLatency=null;this.selectedShip=0;this.nextEntityId=0;this.installFxRelay();this.bindUI();
    }

    installFxRelay(){
      const g=this.game;
      if(g.__coopFxRelayInstalled)return;g.__coopFxRelayInstalled=true;
      const pulse=g.activatePulse.bind(g);
      g.activatePulse=(playerIndex)=>{
        const before=g.pulseEnergy,idx=playerIndex==null?g.localPlayerIndex:playerIndex;
        pulse(playerIndex);
        if(g.onlineRole==="host"&&before>=Starfall.CONFIG.PULSE.maxEnergy&&g.pulseEnergy===0)this.sendFx("pulse",{playerIndex:idx});
      };
      const destroyEnemy=g.destroyEnemy.bind(g);
      g.destroyEnemy=(enemy,byPulse,collision)=>{
        const alive=enemy&&!enemy.dead,x=enemy?.x,y=enemy?.y,color=enemy?.color,type=enemy?.type;
        destroyEnemy(enemy,byPulse,collision);
        if(alive&&enemy?.dead&&g.onlineRole==="host")this.sendFx("enemy_burst",{x:x/g.width,y:y/g.height,color,type,byPulse:Boolean(byPulse)});
      };
      const destroyBoss=g.destroyBoss.bind(g);
      g.destroyBoss=()=>{
        const boss=g.boss,alive=boss&&!boss.dead,x=boss?.x,y=boss?.y;
        destroyBoss();
        if(alive&&boss?.dead&&g.onlineRole==="host")this.sendFx("boss_burst",{x:x/g.width,y:y/g.height});
      };
    }

    sendFx(event,data){if(this.role==="host")this.send({type:"fx",event,data});}
    playFx(event,data){
      const g=this.game;if(!data||g.onlineRole!=="guest")return;
      if(event==="pulse"){
        const source=g.players?.[Number(data.playerIndex)||0]||g.player;if(!source)return;
        g.effects.wave(source.x,source.y,Starfall.CONFIG.PULSE.radius,"#6fffff");g.effects.burst(source.x,source.y,"#7dffff",34,310);g.audio.play("pulse");g.shake=Math.max(g.shake,9);
      }else if(event==="enemy_burst"){
        const x=Number(data.x)*g.width,y=Number(data.y)*g.height;if(!Number.isFinite(x)||!Number.isFinite(y))return;
        const heavy=data.type==="heavy";g.effects.burst(x,y,"#ffb35a",heavy?24:14,heavy?255:185);if(data.byPulse)g.effects.text(x,y,"PULSE!","#8effff",13);g.audio.play("explosion");g.shake=Math.max(g.shake,heavy?5:2);
      }else if(event==="boss_burst"){
        const x=Number(data.x)*g.width,y=Number(data.y)*g.height;if(!Number.isFinite(x)||!Number.isFinite(y))return;
        g.effects.wave(x,y,260,"#ff70dd");for(let i=0;i<5;i+=1){const a=i/5*Math.PI*2;g.effects.burst(x+Math.cos(a)*44,y+Math.sin(a)*30,"#ffb35a",24,290);}g.audio.play("explosion");g.shake=Math.max(g.shake,12);
      }
    }
    ...
    bindUI(){
      ...
      document.getElementById("coop-button").addEventListener("click",()=>this.game.openWorldSelector("coop"));
      document.getElementById("coop-back-button").addEventListener("click",()=>{this.leave();this.game.showScreen("main-menu");});
      document.getElementById("create-room-button").addEventListener("click",()=>this.createRoom());document.getElementById("join-room-button").addEventListener("click",()=>this.joinRoom());
      this.input.addEventListener("input",()=>{this.input.value=this.input.value.toUpperCase().replace(/[^A-Z]/g,"");});this.input.addEventListener("keydown",event=>{if(event.key==="Enter")this.joinRoom();});
      document.getElementById("copy-room-button").addEventListener("click",async()=>{try{await navigator.clipboard.writeText(this.room||"");this.setStatus("ROOM CODE COPIED");}catch(_){this.setStatus(`ROOM: ${this.room}`);}});
    }

    setStatus(message,error){this.status.textContent=message;this.status.classList.toggle("error",Boolean(error));}
    async connect(){
      if(this.socket?.readyState===WebSocket.OPEN)return;
      if(this.socket?.readyState===WebSocket.CONNECTING){await new Promise((resolve,reject)=>{this.socket.addEventListener("open",resolve,{once:true});this.socket.addEventListener("error",reject,{once:true});});return;}
      this.intentionalClose=false;const protocol=location.protocol==="https:"?"wss:":"ws:";this.socket=new WebSocket(`${protocol}//${location.host}/ws`);this.setStatus("CONNECTING…");this.socket.addEventListener("message",event=>this.onMessage(event));this.socket.addEventListener("close",()=>this.onClose());
    ...
    onMessage(event){
      ...
      else if(message.type==="fx"&&this.role==="guest")this.playFx(message.event,message.data);
    ...
    tick(now){
      ...
      if(this.role!=="host"||this.game.onlineRole!=="host")return;
      const buffered=this.socket?.bufferedAmount||0,interval=buffered>16384?125:75;if(now-this.lastStateAt<interval||!this.socket||buffered>65536)return;
      this.lastStateAt=now;this.send({type:"state",state:this.snapshot()});
    }
    ...
    snapshot(){const g=this.game,w=g.width,h=g.height;return{seq:++this.stateSeq,guestInputAck:this.lastGuestInputSeq,environment:g.environment?.id||"neo-shibuya",elapsed:g.elapsed,score:g.score,kills:g.kills,killChain:g.killChain,comboTimer:g.comboTimer,combo:g.combo,bestCombo:g.bestCombo,pulseEnergy:g.pulseEnergy,nextBossTime:g.nextBossTime,finished:g.state==="gameover",players:g.players.map((item,index)=>pick(item,FIELDS.player,w,h,this.idFor(item,"player",index))),playerProjectiles:g.playerProjectiles.map((item,index)=>pick(item,FIELDS.projectile,w,h,this.idFor(item,"shot",index))),enemyProjectiles:g.enemyProjectiles.map((item,index)=>pick(item,FIELDS.projectile,w,h,this.idFor(item,"enemy-shot",index))),enemies:g.enemies.map((item,index)=>pick(item,FIELDS.enemy,w,h,this.idFor(item,"enemy",index))),powerups:g.powerups.map((item,index)=>pick(item,FIELDS.powerup,w,h,this.idFor(item,"powerup",index))),boss:g.boss?pick(g.boss,FIELDS.boss,w,h,"boss"):null,companions:g.companions.map((item,index)=>pick(item,FIELDS.companion,w,h,this.idFor(item,"companion",index)))};}

    applySnapshot(state){
      if(!state||!Array.isArray(state.players))return false;if(Number.isSafeInteger(state.seq)&&state.seq<=this.lastSnapshotSeq)return false;if(Number.isSafeInteger(state.seq))this.lastSnapshotSeq=state.seq;
      const receivedAt=performance.now(),g=this.game,w=g.width,h=g.height;if(typeof state.environment==="string"&&typeof g.setEnvironment==="function"&&state.environment!==g.environment?.id)g.setEnvironment(state.environment,false);
      for(const key of ["elapsed","score","kills","killChain","comboTimer","combo","bestCombo","pulseEnergy","nextBossTime"])if(Number.isFinite(state[key]))g[key]=state[key];
      this.acknowledgeInput(state.guestInputAck,receivedAt);
      g.players=reconcile(g.players,state.players,Starfall.Player,FIELDS.player,w,h,"player",g.localPlayerIndex,receivedAt);g.player=g.players[0];
    ...
    leave(sendMessage){if(sendMessage!==false)this.send({type:"leave"});this.intentionalClose=true;if(this.socket)this.socket.close();this.socket=null;this.role=null;this.room=null;this.lastSnapshotSeq=-1;this.lastSnapshotAck=0;this.pendingInputs.clear();this.localTarget=null;this.controlLatency=null;this.actions.classList.remove("hidden");this.waiting.classList.add("hidden");}
